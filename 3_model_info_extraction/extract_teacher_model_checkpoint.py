'''
3.1 extract teacher model with checkpoint
use to prove teacher model

exec: 
$ python extract_teacher_model_checkpoint.py

output folder: extraction_teacher
...
# 創建資料夾
extraction_teacher_filename = 'extraction_teacher'
...
def hook_fc1(model, input, output):
    ...
    random_index = np.random.randint(0, 1200)
    checkpoint_weight = layer_weight[random_index]
    checkpoint_bias = layer_bias[random_index]
    
    checkpoint_weight = function_scaling(checkpoint_weight)
    checkpoint_bias = function_scaling(checkpoint_bias)
    layer_input = function_scaling(layer_input)
    layer_output = function_scaling(layer_output)
    
    checkpoint_weight.astype(np.int32)
    checkpoint_bias.astype(np.int32)
    layer_input.astype(np.int32)
    layer_output.astype(np.int32)
    
    calculated_output_temp = function_descaling(np.dot(layer_input, checkpoint_weight.T))
    calculated_output = calculated_output_temp + checkpoint_bias
    
    # 將input, output, checkpoint_weight, checkpoint_bias, calculated_output 存成json檔案
    operation_log = {
            'layer': 'fc1',
            'choose_neuron': random_index,
            'input': layer_input.tolist()[0],
            'output': layer_output.tolist()[0],
            'checkpoint_weight': checkpoint_weight.tolist(),
            'checkpoint_bias': checkpoint_bias.tolist(),
            'calculated_output': calculated_output.tolist()[0]
        }
